...
class MinPriorityQueue {
  ...
  _bubbleUp() {
    let i = this.h.length - 1;
    while (i > 0 && this.h[i].data < this.h[Math.floor((i - 1) / 2)].data) {
      [this.h[i], this.h[Math.floor((i - 1) / 2)]] = [
        this.h[Math.floor((i - 1) / 2)],
        this.h[i]
      ];
  ...
  _bubbleDown() {
    ...
    while (
      i < this.h.length &&
      ((this.h[2 * i + 1] && this.h[i].data > this.h[2 * i + 1].data) ||
        (this.h[2 * i + 2] && this.h[i].data > this.h[2 * i + 2].data))
    ) {
      let minIdx;
      if (this.h[2 * i + 1] && !this.h[2 * i + 2]) minIdx = 2 * i + 1;
      else if (this.h[2 * i + 1].data < this.h[2 * i + 2].data)
        minIdx = 2 * i + 1;
      else minIdx = 2 * i + 2;
      [this.h[i], this.h[minIdx]] = [this.h[minIdx], this.h[i]];
      i = minIdx;
...
module.exports = { MinHeap, MaxHeap, MinPriorityQueue };
